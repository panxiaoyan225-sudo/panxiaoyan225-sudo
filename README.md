# 🛠️ Project Focus   ![Visitor Count](https://komarev.com/ghpvc/?username=panxiaoyan225-sudo&color=blue&style=flat-square) 
[🏥 Explore Project Architecture](https://panxiaoyan225-sudo.github.io/panxiaoyan225-sudo/index.html)
* **Automated CI/CD Pipeline Integration:** Engineered high-availability **CI/CD automation** across **Azure DevOps, GitHub Actions, and Google Cloud (Cloud Run)**. Features redundant automated hourly "heartbeat" runs and **real-time Slack alerting** , ensuring 100% pipeline uptime and proactive health management for production ETLs.[🚀 View Pipeline Infographic](https://panxiaoyan225-sudo.github.io/data-pipelines/data_infographic.html) & [ View Pipeline Logic Infographic](https://panxiaoyan225-sudo.github.io/Portfolio_DevOps/infographic.html)
   
* **Production-Grade Orchestration**: Containerized ETL/ELT pipelines using **Docker and Apache Airflow**, with a focus on implementing custom Python validation layers to ensure 100% data integrity for institutional decision-making [📊 View ETL Infographic](https://panxiaoyan225-sudo.github.io/university_etl_Docker/university_infographic.html)
  
* **Distributed Cloud Engineering**: Migrating legacy analytics to high-performance **Databricks/PySpark** environments, utilizing distributed computing to scale healthcare and financial data processing beyond single-node limitations
  
* **AI-Driven Strategic Intelligence**: Orchestrating LLMs and advanced prompt engineering (OmniAI-Toolkit) to streamline complex reporting, transforming multi-source data into high-accuracy, actionable insights for high-stakes environments.
---
*  [View my **Portfolio**](https://xiaoyan-pan-data-analytics-portfolio-170069322954.us-west1.run.app/)                            



